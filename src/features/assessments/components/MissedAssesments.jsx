...
export default function MissedAssessments() {
  ...
  useEffect(() => {
    fetchMissedAssessments();
  }, []);

  const fetchMissedAssessments = async () => {
    try {
      const res = await AssessmentService.MissedAssessmentList();
      setData(res.data ?? res); 
    } catch (error) {
      console.error("Error fetching Missed assessments", error);
      setData([]);
    } finally {
      setLoading(false);
    }
